...
| 第三问 | 1471 | 42.086215 | 0.693126 | 六区微调、正式收口、边界检验及加密验收完成 |
...
最终方案还对五条内部边界执行 $\pm1$--$2$ 环单因素移动，得到 18 个合法候选并
全部完成中精度和正式精度评价。8 个候选功率可行但 $q$ 下降，8 个候选提高
$q$ 但功率不达标，另 2 个两项均不占优，因此保留边界
$(1,5,11,14,20)$。该结果只说明固定规格和有限邻域下的局部合理性。

正式结果位于 `outputs/q3/`，按 01--21 保存完整代码、扫描与敏感性数据、搜索及
收口轨迹、正式和加密比较、六区与逐镜参数、几何验证、论文表格和五张图片；
正式提交文件使用题目原名 `outputs/q3/result3.xlsx`。

边界局部敏感性检验已并入同一套第三问交付，不设独立入口或输出目录：

- `outputs/q3/20_六区边界局部敏感性检验.csv`：记录 18 个合法边界候选的
  中精度、正式精度、相对基准增量与分类；
- `outputs/q3/21_六区边界局部敏感性图.png`：集中展示五条内部边界移动后的
  功率与单位面积输出变化。
...
`outputs/q3/21_六区边界局部敏感性图.png`；其余扫描与轨迹文件用于代码复核。
