...
$$
6.2\ \mathrm{m}\times6.2\ \mathrm{m},
$$

单镜面积为

$$
A=6.2^2=38.44\ \mathrm{m^2}.
$$
...
集热器为半径 $4\ \mathrm{m}$、高度 $8\ \mathrm{m}$ 的圆柱侧面：

$$
x^2+y^2=16,\qquad 82\le z\le90.
$$
...
$$
9{:}00,\quad10{:}30,\quad12{:}00,\quad13{:}30,\quad15{:}00.
$$
...
令竖直方向为 $\boldsymbol k=(0,0,1)$，建立镜面局部坐标系：
...
\boldsymbol q
=
\boldsymbol c_i+a\boldsymbol u_i+b\boldsymbol v_i,
\qquad -3.1\le a,b\le3.1.
$$
...
若一个采样点的入射光和反射光都未被遮挡，则记为有效点。阴影遮挡效率为
...
阴影损失和反射遮挡损失必须在采样点层面取并集，不能直接把两个损失比例相加。
...
R_b
=
\frac{\sqrt{6.2^2+6.2^2}}{2}
\approx4.384\ \mathrm{m}.
$$

只有位于射线前方，并且中心到射线的垂直距离小于约 $2R_b$ 的定日镜，才进入精确的射线—矩形求交检查。
...
$$
\theta_\odot=4.65\ \mathrm{mrad}.
$$

对每条采样光线，根据反射定律计算反射方向，再判断它是否命中圆柱形集热器侧面。

若 256 条光线中有 $N_{\mathrm{hit}}$ 条命中，则

$$
\boxed{
\eta_{\mathrm{trunc},i}
=
\frac{N_{\mathrm{hit}}}{256}
}
$$
...
平均光学效率应先逐镜计算效率乘积，再求平均：

$$
\overline{\eta}_{\mathrm{opt}}(t)
=
\frac{1}{1745}
\sum_{i=1}^{1745}\eta_i(t).
$$
