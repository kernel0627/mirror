...
本文档记录第一问的光学模型推导、射线求交、计算加速、程序结构和验证方法。
建模主线与结果见 [`第一问.md`](第一问.md)，集中公式见
...
| `q1/solve.py` | 合成各项效率，组织 60 个时刻的计算流程并流式累计单镜指标 |
| `q1/aggregate.py` | 汇总全场月平均、年平均和单镜年平均结果 |
| `q1/export.py` | 输出逐时刻、月平均、年平均、单镜年平均、运行配置和论文表格 |
| `q1/plot.py` | 生成月平均组合图和单镜年平均综合效率空间图 |
| `solve_q1.py` | 保留原有运行方式的轻量兼容入口 |

程序不保存全部单镜逐时刻数据，只保留 1745 行单镜年平均结果。论文生成两张结果图：月平均综合光学效率与单位面积输出热功率组合图，以及单镜年平均综合光学效率空间分布图。
